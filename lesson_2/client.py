...
class CommandsRequest(metaclass=ClientVerifier):
    ...
    def commands(self, server_socket, user_name):
        ...
        while True:
            comm = input("\nВведите команду: ")
            if comm == "mess":
                message_to_send = message_creator(sender=user_name)
                send_message(client=server_socket, message=message_to_send)
                ...
                send_message(client=server_socket, message=exit_message)
                time.sleep(0.5)
                CLIENT_LOGGER.info(f"Завершение работы по команде пользователя - {user_name}")
                break
...
class ParsingMessage(metaclass=ClientVerifier):
    ...
    def parsing(self):
        ...
        while True:
            ...
            message = incoming_message(client=self)
...
def main():
    ...
    try:
        ...
        message_reader = threading.Thread(target=ParsingMessage.parsing, args=(soc, ))
        ...
        message_sender = threading.Thread(target=CommandsRequest.commands, args=('', soc, client_name, ))
